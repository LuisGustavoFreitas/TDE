...
def calcularTroco(valorRecebido, valorProduto): #funcao para calcular o troco 
    ...
    trocoDistribuicao = {}
    for moeda in sorted(caixa, reverse=True):  #'sorted(caixa.keys(), reverse=True)' obtem todas as chaves do dicionario caixa e ordena em ordem decrescente
...
def main():
    while True:
        ...
        valorRecebido = float(input("Insira o valor pago: R$ "))
        troco, falta = calcularTroco(valorRecebido, produto["preco"]) #calcular o troco conforme a entrada do usuario 
        ...
        if troco is None:  #  significa que o valor pago foi insuficiente ou não há troco disponível, o programa informa ao usuario o valor faltante
            print(f"Valor insuficiente ou troco não disponível. Faltam R$ {abs(falta):.2f}")
            continue
